 ...
 * @author mvk
 */
public final class SkAlarmFilterByLevel
    implements ITsFilter<ISkAlarm> {

  /**
   * Идентификатор типа фильтра {@link ITsSingleFilterFactory#id()},
   */
  public static final String TYPE_ID = STD_FILTERID_ID_PREFIX + ".AlarmFilterByLevel"; //$NON-NLS-1$
  ...
  public static final ITsSingleFilterFactory<ISkAlarm> FACTORY =
      new AbstractTsSingleFilterFactory<>( TYPE_ID, ISkAlarm.class ) {
        ...
        protected ITsFilter<ISkAlarm> doCreateFilter( IOptionSet aParams ) {
          ...
          return new SkAlarmFilterByLevel( op, constant );
        }
      };
   ...
   * @param aConst {@link IAtomicValue} - константа для сравнения имеющая тип {@link EAtomicType#INTEGER}
   * @throws TsNullArgumentRtException любой аргумент = null
   */
  public SkAlarmFilterByLevel( EAvCompareOp aOp, IAtomicValue aConst ) {
    TsNullArgumentRtException.checkNulls( aOp, aConst );
    TsIllegalArgumentRtException.checkFalse( aConst.atomicType() == EAtomicType.INTEGER );
    op = aOp;
    constant = aConst;
  }
  ...
  public boolean accept( ISkAlarm aAlarm ) {
    ...
    IAtomicValue author = AvUtils.avInt( aAlarm.level() );
