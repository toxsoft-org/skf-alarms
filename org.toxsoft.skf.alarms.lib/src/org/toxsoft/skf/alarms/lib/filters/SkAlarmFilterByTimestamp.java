 ...
 * @author mvk
 */
public final class SkAlarmFilterByTimestamp
    implements ITsFilter<ISkAlarm> {

  /**
   * Идентификатор типа фильтра {@link ITsSingleFilterFactory#id()},
   */
  public static final String TYPE_ID = STD_FILTERID_ID_PREFIX + ".AlarmFilterByTimestamp"; //$NON-NLS-1$
  ...
  public static final ITsSingleFilterFactory<ISkAlarm> FACTORY =
      new AbstractTsSingleFilterFactory<>( TYPE_ID, ISkAlarm.class ) {
        ...
        protected ITsFilter<ISkAlarm> doCreateFilter( IOptionSet aParams ) {
          ...
          return new SkAlarmFilterByTimestamp( op, constant );
        }
      };
   ...
   * @param aConst {@link IAtomicValue} - константа для сравнения имеющая тип {@link EAtomicType#TIMESTAMP}
   * @throws TsNullArgumentRtException любой аргумент = null
   */
  public SkAlarmFilterByTimestamp( EAvCompareOp aOp, IAtomicValue aConst ) {
    TsNullArgumentRtException.checkNulls( aOp, aConst );
    TsIllegalArgumentRtException.checkFalse( aConst.atomicType() == EAtomicType.TIMESTAMP );
    op = aOp;
    constant = aConst;
  }
  ...
  public boolean accept( ISkAlarm aAlarm ) {
    ...
    IAtomicValue timestamp = AvUtils.avTimestamp( aAlarm.timestamp() );
    return c.avCompare( timestamp, op, constant );
  }
