 ...
 * @author mvk
 */
public final class SkAlarmFilterByAuthor
    implements ITsFilter<ISkAlarm> {

  /**
   * Идентификатор типа фильтра {@link ITsSingleFilterFactory#id()},
   */
  public static final String TYPE_ID = STD_FILTERID_ID_PREFIX + ".AlarmFilterByAuthor"; //$NON-NLS-1$
  ...
  public static final ITsSingleFilterFactory<ISkAlarm> FACTORY =
      new AbstractTsSingleFilterFactory<>( TYPE_ID, ISkAlarm.class ) {
        ...
        protected ITsFilter<ISkAlarm> doCreateFilter( IOptionSet aParams ) {
          ...
          return new SkAlarmFilterByAuthor( op, constant );
        }
      };
   ...
   * @param aConst {@link IAtomicValue} - константа для сравнения имеющая тип {@link EAtomicType#VALOBJ}({@link Skid})
   * @throws TsNullArgumentRtException любой аргумент = null
   * @throws TsIllegalArgumentRtException недопустимый тип
   */
  public SkAlarmFilterByAuthor( EAvCompareOp aOp, IAtomicValue aConst ) {
    ...
    TsIllegalArgumentRtException.checkFalse( aConst.asValobj() instanceof Skid );
    op = aOp;
    constant = aConst;
  }
  ...
  public boolean accept( ISkAlarm aAlarm ) {
    ...
    IAtomicValue author = AvUtils.avValobj( aAlarm.authorId() );
    return c.avCompare( author, op, constant );
  }
